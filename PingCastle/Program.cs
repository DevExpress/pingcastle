 ...
 ***********   %*** -	Customers, visit https://www.netwrix.com/support.html";
        ...
        private bool ParseCommandLine(string[] args)
        {
            ...
            {
                WriteInRed("You must choose at least one value among --healthcheck --entraid --hc-conso --advanced-export --advanced-report --nullsession --carto");
                DisplayHelp();
                return false;
            }
            ...
            foreach (var action in requestedActions)
            {
                if (requiredSettings.ContainsKey(action))
                {
                    ...
                    if (r.Length > 0)
                    {
                        ...
                        {
                            if (state == DisplayState.MainMenu)
                            {
                                // User chose to go back to main menu, restart interactive mode
                                requestedActions.Clear();
                                return RunInteractiveMode();
                            }
                            else if (state == DisplayState.Exit)
                            {
                                // User/task wants to exit application
                                return false;
                            }
                            else
                            {
            ...
            while (states.Count > 0 && states.Peek() != DisplayState.Run)
            {
                switch (state)
                {
                    ...
                    case DisplayState.AskForScannerParameter:
                        state = settings.QueryScannerForAdditionalParameterInInteractiveMode();
                        break;
                ...
                if (state == DisplayState.Exit)
                {
                    ...
                    // If there are tasks to run, exit the menu loop to run them
                    if (requestedActions.Count > 0)
                    {
                        break;
                    }
                    // If we're exiting from the main menu, exit the application completely
                    if (currentState == DisplayState.MainMenu)
                    {
                        return false;
                    }
                    // Otherwise go back to previous menu if available
                    if (states.Count > 0)
                        state = states.Peek();
                }
                else
        ...
        DisplayState DisplayMainMenu()
        {
            ...
            List<MenuItem> choices = new List<MenuItem>() {
                ...
                new MenuItem("entraid","Score the risk of Entra ID", "This is the main functionality of PingCastle. In a matter of minutes, it produces a report which will give you an overview of your EntraID security."),
        ...
        private static void DisplayHelp()
        {
            ...
            Console.WriteLine("");
            Console.WriteLine("  --scanner <type>    : perform a scan on one or all computers of the domain (using --server)");
            Console.WriteLine("");
            ...
            Console.WriteLine("    --services <names>: for anti-virus scan, include these services as anti-virus services");
            Console.WriteLine("                      : names: a comma separated list of service names");
            Console.WriteLine("");
            ...
            Console.WriteLine("");
            Console.WriteLine("Common options when connecting to Entra ID");
